...
#### 2.7 GDB/MI 控制器
- **职责**：封装对 GDB 的异步交互，使用 GDB/MI2 协议（`--interpreter=mi2`）。
  - **启动**：`start(firmware_path)`，启动 GDB 进程，连接目标，加载固件。
  - **发送 MI 命令**：`send_mi(mi_command)`，发送 MI 命令并等待结果记录。
  - **继续执行**：`exec_continue()`，发送 `-exec-continue`，立即返回（异步，不阻塞）。
  - **中断执行**：`interrupt()`，发送 `-exec-interrupt`（Windows 上回退到 OpenOCD telnet halt）。
  - **停止**：终止进程。
- **实现要点**：
  - 使用 GDB/MI2 异步协议，事件驱动，无需轮询 `(gdb)` 提示符。
  - 后台线程持续读取 GDB stdout，解析 MI 记录，分发到事件队列和命令 Future。
  - `continue` 后 GDB 立即返回 `^running`，不再阻塞。
  - GDB 主动推送 `*stopped`/`*running` 异步事件，无需轮询。
  - 跨平台可靠：Unix 上 `-exec-interrupt` 直接生效；Windows 上自动回退到 OpenOCD telnet halt。

#### 2.8 RTT 客户端
- **职责**：连接 OpenOCD 开启的 RTT TCP 端口，按行缓冲读取实时日志。
  - **连接**：`connect(host, port)` — 连接到 RTT 服务器。
  - **读取**：`read_lines(max_lines)` — 读取最多指定行数的日志。
  - **关闭**：`close()` — 关闭连接。
- **实现要点**：
  - 使用 `socket` TCP 连接，后台线程持续读取。
  - 线程安全：通过锁保护行缓冲区。
  - RTT 是非致命功能：如果固件不支持 RTT，调试会话照常运行。

#### 2.9 特性边界
- RTT 已实现并集成到调试会话中，作为可选增强。
- RTT 详细设计见 `RTT特性.md`。
