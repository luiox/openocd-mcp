...
openocd-mcp 是一个 MCP 服务器，它允许 AI 客户端通过自然语言调用嵌入式烧录与调试功能。其核心设计理念是 **完全复用开发者已有的 VS Code 调试配置（launch.json）**，让 AI 能够像开发者一样理解项目中的调试目标，并以最少的参数完成烧录、调试、日志读取等操作。
...
- **关键逻辑**：
  ...
  - 对于 `debug_command`、`debug_stop`、`read_rtt`、`debug_status`，调用调试会话管理器或对应控制器。
- **错误处理**：所有调用均包装在 try-except 中，返回以 `"Error: "` 开头的友好错误信息。

#### 2.3 全局配置管理器
- **职责**：管理服务器启动时传入的全局配置（OpenOCD 路径、GDB 路径、RTT 端口等）。
- **数据来源**：命令行参数（如 `--openocd-path`）或环境变量。可扩展支持从 `~/.openocd-mcp/config.json` 读取默认值。
- **提供接口**：`get_openocd_path()`, `get_gdb_path()`, `get_rtt_port()` 等。
...
- **职责**：管理当前的调试会话（最多一个）。提供以下功能：
  - `start_session(config_name, firmware_path_override)`：启动 OpenOCD 和 GDB，建立 RTT 连接，保存进程对象和 socket。
  - `stop_session()`：终止所有子进程，关闭 socket，清理状态。
  - `get_status()`：返回当前会话状态（是否活跃、使用的配置、进程 PID、RTT 状态等）。
...
- **职责**：封装对 OpenOCD 的所有操作。
  ...
  - **启动 GDB 服务器**：`start_server(config_files, enable_rtt)`，启动常驻 OpenOCD 进程，并返回进程对象。
  - **停止服务器**：终止进程。
- **实现细节**：
  - 使用 `subprocess.Popen` 或 `asyncio.create_subprocess_exec` 管理进程。
  - 对于一次性烧录，需等待进程结束并捕获 stdout/stderr。
  - 启动服务器时，需额外传递 RTT 配置（如 `-f rtt.cfg`），若用户未提供 RTT 脚本，可自动生成临时脚本或使用默认配置。
...
- **职责**：连接 OpenOCD 开启的 RTT TCP 端口，读取实时日志。
  - **连接**：`connect(host, port)`，创建 socket 连接。
  - **读取**：`read_lines(max_lines)`，从 socket 非阻塞读取最多指定行数的数据（可设置缓冲区）。
  - **关闭**：关闭 socket。
- **实现**：使用 Python 的 `socket` 或 `asyncio.open_connection`，将读取的数据按行分割并返回最新行。注意处理连接断开和重连。
...
2. **AI 调用 `debug_start` 传入 `config_name="Debug mbootloader"`**
   ...
   - 调试会话管理器 → OpenOCD控制器：启动 OpenOCD 服务器（使用配置中的 `configFiles`，并加载 RTT 脚本）。
   - 调试会话管理器 → GDB控制器：启动 GDB，加载固件，自动连接并运行到入口（若配置中有 `runToEntryPoint`）。
   - 调试会话管理器 → RTT客户端：连接 OpenOCD 的 RTT 端口。
   - 保存会话状态，返回成功信息给 AI。
...
4. **用户想查看日志**
   - AI 调用 `read_rtt`。
   - 工具调度器 → 调试会话管理器：确保会话活跃且 RTT 已连接。
   - 调试会话管理器 → RTT客户端：读取最新日志，返回给 AI。
...
- **极简 AI 接口**：AI 只需知道项目目录和配置名称，即可执行所有操作（烧录、调试、命令、读日志）。参数数量最小化。
...
openocd-mcp 的架构围绕 **复用现有配置** 和 **极简 AI 接口** 展开，通过明确的模块划分和清晰的职责，实现了一个可靠、可扩展的 MCP 服务器。开发者无需额外学习，AI 只需掌握两个核心信息（项目目录和配置名称），即可完成嵌入式开发的全闭环操作。